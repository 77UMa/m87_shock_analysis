...
def visualize_shock_projection(roi_data, shock_properties, snapshot_name, shock_plot_filename):
    ...
    # 定义绘图范围和直方图的精细度
    plot_range = 50 # 例如，显示 +/- 200 r_g 的范围
    bins = 256       # 直方图的网格数
...
if __name__ == '__main__':
    ...
    SAFE_MAX_WORKERS = 14 #14是稳健的，15可能会有问题
    ...
    print(f"Using ProcessPoolExecutor with a maximum of {SAFE_MAX_WORKERS} concurrent workers to stay within memory limits.")
    
    # ProcessPoolExecutor 会自动处理 config 的传递
    # 我们使用 partial 来固定 config 参数
    task_func = partial(analyze_snapshot_full_pipeline, config=config)
    # ProcessPoolExecutor.map 需要的函数只接受一个迭代参数，所以我们只准备文件名列表
    tasks = [(filename, config) for filename in file_list]
    filenames = [f for f, _ in tasks]

    start_total_time = time.time()
    
    # --- 2. 【核心修改】使用 ProcessPoolExecutor 代替 multiprocessing.Pool ---
    # 它使用 with 语句，能更可靠地管理子进程的创建和销毁
    try:
        # --- 【核心修改】: 添加 max_tasks_per_child=1 ---
        # 这个参数会强制每个子进程在完成1个任务后就自动销毁并重启。
        with ProcessPoolExecutor(max_workers=SAFE_MAX_WORKERS) as executor:
            
            print(f"--- Submitting {len(tasks)} tasks with process recycling enabled ---")
            
            results = executor.map(task_func, filenames)
            
            for i, result in enumerate(results):
                print(f"  > Task {i + 1}/{len(filenames)} has completed.")
